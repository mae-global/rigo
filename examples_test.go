...
func Benchmark_SimpleExampleNumberHandlers(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		ctx.Display("sphere.tif","file","rgb")
		ctx.Format(320,240,1)
		ctx.Projection(PERSPECTIVE,RtToken("fov"),RtFloat(30))
		ctx.Translate(0,0,6)
		ctx.WorldBegin()
		ctx.LightSource("ambientlight",RtToken("intensity"),RtFloat(0.5))
		ctx.LightSource("distantlight",RtToken("intensity"),RtFloat(1.2),RtToken("form"),RtIntArray{0,0,-6},RtString("to"),RtIntArray{0,0,0})
		ctx.Color(RtColor{1,0,0})
		ctx.Sphere(1,-1,1,360)
		ctx.WorldEnd()
		ctx.End()
