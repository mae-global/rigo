...
const (
	...
	PrototypesBloomFilterKeys int = {{.Ri.FilterKeys}}
)


var (
	PrototypesBloomFilterData = []int{ {{range $i := .Ri.FilterData}}{{$i}},{{end}} }

	PrototypesData string = "{{.Ri.Prototypes}}"

	PrototypesIndices = []int{ {{range $i := .Ri.PrototypesIndices}}{{$i}},{{end}} }
)


func RiBloomFilter() *BloomFilter {
	bits := make([]int,len(PrototypesBloomFilterData))
	copy(bits,PrototypesBloomFilterData)
...
func RiPrototypes() map[RtName]*PrototypeInformation {
	...
	for i,_ := range PrototypesIndices {
		if (i + 1) >= len(PrototypesIndices) {
			idx := PrototypesIndices[i]
			statements = append(statements,PrototypesData[ idx : ])
		} else {	
			idx := PrototypesIndices[i]
			statements = append(statements,PrototypesData[ idx : PrototypesIndices[i + 1] ])
		}
	}
